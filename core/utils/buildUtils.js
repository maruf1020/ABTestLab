...
export async function buildTest(website, test, variation, testType) {
  ...
  if (testType !== "Multi-touch") {
    ...
    const touchPoints = testInfo.touchPoints || [];
    variationDirList = touchPoints.map((touchPoint) =>
      getVariationDirForTouchPoint(website, test, touchPoint, variation)
    );
  }
  await Promise.all(
    variationDirList.map((variationDir) => buildVariation(variationDir))
  );
}
...
export async function buildMultipleTest(selectedVariations) {
  ...
  await Promise.all(
    selectedVariations.map(async (v) => {
      let variationDirList = [];
