...
import {
  ...
  selectTest,
  selectVariation,
  selectMultipleWebsites,
  selectMultipleTests,
  selectMultipleVariations,
} from "./selectors.js";
...
export async function handleLatestTest(lastTest, goBack) {
  ...
  if (!isGroupTest) {
    choices.splice(1, 0, {
      title: chalk.blueBright("🔄 Change Variation"),
      value: "changeVariation",
    });
    choices.splice(2, 0, {
      title: chalk.blueBright("🔄 Change Test"),
      value: "changeTest",
    });
  }
  ...
  switch (action) {
    case "run":
      if (lastTest.tests.length > 1) {
        await startMultipleTest(
          lastTest.tests.map((test) => ({
        ...
        await startTest(
          testData.websiteName,
          testData.testName,
          testData.variationName,
          testData.testType
        );
      }
      break;
    case "changeVariation":
      const variationName = await selectVariation(
        testData.websiteName,
        testData.testName,
        () => handleLatestTest(lastTest, goBack)
      );
    ...
    case "changeTest":
      return await handleTestSelectionWithHistory(
        testData.websiteName,
        lastTest,
        goBack
      );
...
export async function viewTestHistory(history, viewType, goBack) {
  ...
  const hasMultiTouchTest = filteredHistory.some((entry) =>
    entry.tests.some((test) => test.testType === "Multi-touch")
  );
  ...
  tableHeaders.push(kleur.green("Variation"), kleur.green("Last Run"));
  ...
  for (const entry of filteredHistory) {
    ...
    for (const test of entry.tests) {
      if (test.testType === "Multi-touch") {
        ...
        row.push(test.testType, test.websiteName, test.testName);

        if (hasMultiTouchTest) {
          row.push("-");
        }

        row.push(test.variationName, new Date(entry.lastRun).toLocaleString());
...
export async function groupTestMenu(history, goBack) {
  const choices = [
    ...
    {
      title: chalk.blueBright("🔄 Run Existing Group test"),
      value: "runHistory",
    },
    { title: chalk.magenta("🔙 Back"), value: "back" },
    { title: chalk.red("❌ Exit"), value: "exit" },
...
async function handleTestSelection(selectedWebsite, goBack) {
  ...
  if (typeof selectedTest === "string") {
    await handleTestDetails(selectedWebsite, selectedTest, () =>
      handleTestSelection(selectedWebsite, goBack)
    );
  }
}
...
async function handleTestSelectionWithHistory(
  selectedWebsite,
  lastTest,
  goBack
) {
  const selectedTest = await selectTest(selectedWebsite, () =>
    handleLatestTest(lastTest, goBack)
  );
  if (typeof selectedTest === "string") {
    await handleTestDetailsWithHistory(
      selectedWebsite,
      selectedTest,
      lastTest,
      goBack
...
async function handleTestDetailsWithHistory(
  selectedWebsite,
  selectedTest,
  lastTest,
  goBack
  ...
  const selectedVariation = await selectVariation(
    selectedWebsite,
    selectedTest,
    () => handleTestSelectionWithHistory(selectedWebsite, lastTest, goBack)
  );
  if (typeof selectedVariation === "string") {
    const testInfo = await getTestInfo(selectedWebsite, selectedTest);
    await startTest(
...
async function runGroupFromHistory(history, goBack) {
  ...
  if (singleTestHistory.length < 1) {
    console.log(
      kleur.yellow("Not enough single tests in history to create a group test.")
    );
    return;
  }
  ...
  const { selectedTests } = await prompts({
    ...
    onRender() {
      if (
        ...
        !this.value.some(
          (option) =>
            option.value && option.value.tests && option.value.tests[0]
        )
      )
        return;
      ...
      if (lastOptionsState.length === 0) {
        ...
        const lastModifiedOption = currentOptionsState.find((option) =>
          lastOptionsState.some(
            (prevOption) =>
              ...
              prevOption.value &&
              option.value.tests &&
              prevOption.value.tests &&
              option.value.tests[0] &&
              prevOption.value.tests[0] &&
              prevOption.value.tests[0].websiteName ===
                option.value.tests[0].websiteName &&
              prevOption.value.tests[0].testName ===
                option.value.tests[0].testName &&
              prevOption.value.tests[0].variationName ===
                option.value.tests[0].variationName &&
              prevOption.value.tests[0].testType ===
                option.value.tests[0].testType &&
          ...
          .some((option) => {
            return (
              currentOptionsState
                ...
                .filter(
                  (selectedOption) =>
                    selectedOption.value.tests[0].websiteName ===
                      option.value.tests[0].websiteName &&
                    selectedOption.value.tests[0].testName ===
                      option.value.tests[0].testName
                ).length > 1
            );
          });
        ...
        selectedOptions.forEach((selectedOption) => {
          if (
            ...
            !selectedOption.value.tests ||
            !selectedOption.value.tests[0] ||
            !lastModifiedOption.value ||
            !lastModifiedOption.value.tests ||
            !lastModifiedOption.value.tests[0]
          )
            return;

          const { websiteName, testName, variationName } =
            selectedOption.value.tests[0];
          if (
            websiteName === lastModifiedOption.value.tests[0].websiteName &&
            testName === lastModifiedOption.value.tests[0].testName
          ) {
            if (
              variationName !== lastModifiedOption.value.tests[0].variationName
            ) {
...
async function runMultipleTests(goBack) {
  ...
  const selectedVariations = await selectMultipleVariations(
    selectedTests,
    goBack
  );
  if (selectedVariations.length === 0) {
    console.log(
      kleur.yellow("No variations selected. Returning to main menu.")
    );
    return;
  }
...
export async function startTest(website, test, variation, testType) {
  ...
  if (testType === "Multi-touch") {
    ...
    touchPoints.forEach((touchPoint, index) => {
      if (index === 0) {
        table.push([
          kleur.green(testType),
          kleur.green(website),
          kleur.green(test),
          kleur.green(touchPoint),
          kleur.green(variation),
        ]);
      } else {
        table.push([
...
export async function startMultipleTest(selectedVariations) {
  const testDirList = selectedVariations.map((v) =>
    path.join(ROOT_DIR, v.website, v.test)
  );
  const testInfoList = await Promise.all(
    testDirList.map((testDir) => fs.readJson(path.join(testDir, "info.json")))
  );
  ...
  selectedVariations.forEach((v) => {
    if (v.testType === "Multi-touch") {
      const touchPoints =
        testInfoList.find((t) => t.name === v.test).touchPoints || [];
      touchPoints.forEach((touchPoint, index) => {
        if (index === 0) {
          table.push([
            kleur.green(v.testType),
            kleur.green(v.website),
            kleur.green(v.test),
            kleur.green(touchPoint),
            kleur.green(v.variation),
          ]);
        } else {
          table.push([
            "",
  ...
  log(
    `Active variations: ${selectedVariations.map((v) => "website: " + v.website + " test: " + v.test + " variation: " + v.variation).join(", ")}`
  );
