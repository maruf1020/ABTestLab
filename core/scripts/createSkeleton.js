...
export async function createSkeleton() {
  ...
  if (
    !fs.existsSync(variationTemplateDir) ||
    !variationFiles.every((file) =>
      fs.existsSync(path.join(variationTemplateDir, file))
    )
  ) {
    throw new Error(
      "Template files are missing or templates directory doesn't exist"
    );
  }

  // Copy template files to variation directory
  await Promise.all(
    variationFiles.map(async (file) => {
      await fs.copyFile(
  ...
  const targetingTemplateDir = path.join(
    __dirname,
    "..",
    "templates",
    "targeting"
  );
  const targetingFiles = [
    "customJS.js",
    "elementChecker.json",
    "urlChecker.json",
  ];
  ...
  // Verify template files exist
  if (
    !fs.existsSync(targetingTemplateDir) ||
    !targetingFiles.every((file) =>
      fs.existsSync(path.join(targetingTemplateDir, file))
    )
  ) {
    throw new Error(
      "Template files are missing or templates directory doesn't exist"
  ...
  const targetMetTemplateDir = path.join(
    __dirname,
    "..",
    "templates",
    "targetMet"
  ...
  // Verify template files exist
  if (
    !fs.existsSync(targetMetTemplateDir) ||
    !targetMetFiles.every((file) =>
      fs.existsSync(path.join(targetMetTemplateDir, file))
    )
  ) {
    throw new Error(
      "Template files are missing or templates directory doesn't exist"
