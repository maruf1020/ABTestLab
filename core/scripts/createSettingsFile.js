...
export async function createSettingsFile() {
  ...
  const defaultSettings = {
    ...
    _comments: {
      ...
      jsReload:
        "If true, the page will reload when CSS changes are detected. If false, CSS changes will be applied without reloading.",
      displayUI:
        "If true, the test helper UI will be displayed on the browser. If false, the UI will be hidden.",
      maxHistoryRecords:
        "The maximum number of history records to keep in the history panel.",
      bundler: {
        ...
        generateMinifiedJS:
          "If true, the bundler will generate a minified JS file.",
        generateMinifiedCSS:
          "If true, the bundler will generate a minified CSS file.",
        generateJSWithCSS:
          "If true, the bundler will generate a JS file with CSS.",
        generateMinifiedJSWithCSS:
          "If true, the bundler will generate a minified JS file with minified CSS.",
      },
    },
  };
  ...
  try {
    ...
    console.error(
      chalk.red(`Failed to create settings.json file: ${error.message}`)
    );
  }
}
