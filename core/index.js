...
const userFriendlyNames = {
  ...
  init: chalk.yellowBright(
    "🛠️  Initialize project (use once on project setup)"
  ),
...
program
  .version("1.0.7")
  .description("A CLI tool for A/B testing directly from a local machine");
...
async function promptUser() {
  const response = await prompts({
    ...
    suggest: (input, choices) =>
      Promise.resolve(
        choices.filter(
          (choice) => choice.title.toLowerCase().includes(input.toLowerCase()) // Match anywhere
        )
      ),
  });
