(() => {
  ...
  const asset = {
    ...
    leftArrow: `
        ...
        `,
    rightArrow: `
        <svg style="padding: 8px;" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" fill="#000000" height="14px" width="14px" version="1.1" id="Layer_1" viewBox="0 0 330 330" xml:space="preserve">
  ...
  function getPopUpElement(initialTestInfo) {
    ...
    popUp
      .querySelector(".ab--pilot-test-details-ui-main-header-minimise")
    ...
    popUp
      .querySelector(".ab--pilot-test-details-ui-main-header-full-screen")
      .addEventListener("click", function () {
    ...
    popUp
      .querySelector(".ab--pilot-test-details-ui-main-header-short-screen")
      .addEventListener("click", function () {
    ...
    popUp
      .querySelector(".ab--pilot-test-details-ui-main-header-close")
      .addEventListener("click", function () {
        waitForElem(".ab--pilot-test-details-ui-opener", function ([opener]) {
          popUp.classList.remove("ab--test-pilot-open");
          setTimeout(() => {
    ...
    popUp
      .querySelectorAll(
        ".ab--test-pilot-accordion .ab--test-pilot-accordion-header"
      )
    ...
    function updateNavigationButtons() {
      ...
      const currentActive = popUp.querySelector(
        ".ab--test-pilot-nav-button-active"
      );
      const buttons = Array.from(
        popUp.querySelectorAll(".ab--test-pilot-nav-button")
      );
      const activeIndex = buttons.indexOf(currentActive);
      const total = buttons.length;
      const visibleItem =
        buttons.length < MAX_DISPLAYED_ITEMS
          ? buttons.length
          : MAX_DISPLAYED_ITEMS;
      ...
      if (isNextHidden) {
        popUp.querySelector(
          ".ab--pilot-test-details-ui-details-navigation"
        ).scrollLeft = popUp.querySelector(
          ".ab--pilot-test-details-ui-details-navigation"
        ).scrollWidth;
      }
    ...
    popUp
      .querySelectorAll(".ab--pilot-test-details-ui-details-navigation-button")
    ...
    popUp
      .querySelectorAll(".ab--test-pilot-nav-button")
      .forEach(function (button) {
        button.addEventListener("click", function () {
          popUp
            .querySelectorAll(".ab--test-pilot-nav-button")
    ...
    popUp
      .querySelector(".ab--pilot-test-details-ui-divider-close-button")
      .addEventListener("click", function () {
        popUp.classList.remove("ab--details-visible");
      });

    popUp
      .querySelectorAll(".ab--pilot-test-details-ui-main-body-table-row")
    ...
    const searchInput = popUp.querySelector(
      ".ab--pilot-test-details-ui-main-header-search input"
    );
    searchInput.addEventListener("input", function () {
      ...
      popUp
        ...
        .forEach(function (row) {
          const title = row
            .querySelector(
              ".ab--pilot-test-details-ui-main-body-table-row-title span"
            )
            .innerText.toLowerCase();
          const status = row
            .querySelector(
              ".ab--pilot-test-details-ui-main-body-table-row-status span"
            )
            .innerText.toLowerCase();
    ...
    popUp
      .querySelectorAll(
        ".ab--pilot-test-details-ui-main-body-table-head-title-button"
      )
      .forEach(function (button) {
        let isAscending = true;

        button.addEventListener("click", function () {
          const header = this.closest(
            ".ab--pilot-test-details-ui-main-body-table-head"
          );
          const rows = Array.from(
            popUp.querySelectorAll(
              ".ab--pilot-test-details-ui-main-body-table-row"
            )
          );
          const index = Array.from(header.children).indexOf(
            this.closest("div")
          );
  ...
  function updateTestDetailsData(data, popUp) {
    const weatingMessage = popUp.querySelector(
      ".ab--test-pilot-waiting-message"
    );
    const accordionWrapper = popUp.querySelector(".ab--test-pilot-accordion");
    const navigation = popUp.querySelector(
      ".ab--pilot-test-details-ui-details-navigation-wrapper"
    );

    const customJSConditionHeader = popUp.querySelector(
      ".ab--test-pilot-accordion-header.ab--test-pilot-custom-js"
    );
    const customJSConditionContent = popUp.querySelector(
      ".ab--test-pilot-contents-wrapper.ab--test-pilot-custom-js"
    );

    const cssCheckerHeader = popUp.querySelector(
      ".ab--test-pilot-accordion-header.ab--test-pilot-css-checker"
    );
    const cssCheckerContent = popUp.querySelector(
      ".ab--test-pilot-contents-wrapper.ab--test-pilot-css-checker"
    );

    const urlCheckerHeader = popUp.querySelector(
      ".ab--test-pilot-accordion-header.ab--test-pilot-url-checker"
    );
    const urlCheckerContent = popUp.querySelector(
      ".ab--test-pilot-contents-wrapper.ab--test-pilot-url-checker"
    );

    const message = popUp.querySelector(
      ".ab--pilot-test-details-ui-details-message-content"
    );
    if (!data) {
      ...
      const targetingDetails =
        data.targetingDetails || data.parentTargetingDetails;
      const customJSCondition = targetingDetails.find(
        (item) => item.type === "customJS"
      );
      const cssChecker = targetingDetails.find(
        (item) => item.type === "elementChecker"
      );
      const urlChecker = targetingDetails.find(
        (item) => item.type === "urlChecker"
      );

      customJSConditionContent.querySelector(
        ".ab--test-pilot-contents-details"
      ).innerHTML = `
            ${customJSCondition ? customJSCondition.messages.map((item) => `<li>${item}</li>`).join("") : "No Details"}`;

      cssCheckerContent.querySelector(
        ".ab--test-pilot-contents-details"
      ).innerHTML = `
            ${cssChecker ? cssChecker.messages.map((item) => `<li>${item}</li>`).join("") : "No Details"}`;

      urlCheckerContent.querySelector(
        ".ab--test-pilot-contents-details"
      ).innerHTML = `
      ...
      customJSConditionContent.querySelector(
        ...
        customJSCondition.status === true
          ? getBadgeHTML("plain", "Active", "success")
          : getBadgeHTML("plain", "Inactive", "danger");

      cssCheckerContent.querySelector(".ab--test-pilot-badge").outerHTML =
        cssChecker.status === true
          ? getBadgeHTML("plain", "Active", "success")
          : getBadgeHTML("plain", "Inactive", "danger");

      urlCheckerContent.querySelector(".ab--test-pilot-badge").outerHTML =
        urlChecker.status === true
          ? getBadgeHTML("plain", "Active", "success")
          : getBadgeHTML("plain", "Inactive", "danger");

      customJSConditionHeader.querySelector(".ab--test-pilot-badge").outerHTML =
        customJSCondition.status === true
          ? getBadgeHTML("minimal", "Active", "success")
          : getBadgeHTML("minimal", "Inactive", "danger");

      cssCheckerHeader.querySelector(".ab--test-pilot-badge").outerHTML =
        cssChecker.status === true
          ? getBadgeHTML("minimal", "Active", "success")
          : getBadgeHTML("minimal", "Inactive", "danger");

      urlCheckerHeader.querySelector(".ab--test-pilot-badge").outerHTML =
        urlChecker.status === true
          ? getBadgeHTML("minimal", "Active", "success")
          : getBadgeHTML("minimal", "Inactive", "danger");
        ...
        navigation
          ...
          .forEach(function (button) {
            button.addEventListener("click", function () {
              ...
              const touchPointData = testsDetailsData[id]
                ? testsDetailsData[id]
                : testsDetailsData[parentId].tests.find(
                    (item) => item.id === id
                  );
  ...
  function updateTableUI(key, value) {
    ...
    if (!row) return;
    const statusHTML = row.querySelector(
      ".ab--pilot-test-details-ui-main-body-table-row-status"
    );
    statusHTML.innerHTML = getBadgeHTML(
      "boxy",
      value.status,
      value.status === "Active" ? "success" : "danger"
    );
  ...
  function mainJs(body, initialTestInfo) {
    ...
    const observedTestsDetailsData = new Proxy(testsDetailsData, {
      set(target, key, value) {
        if (!target.hasOwnProperty(key)) {
          ...
          const current = Object.keys(observedTestsDetailsData).filter(
            (item) => observedTestsDetailsData[item].status === "Active"
          ).length;
          opener.querySelector("span:nth-child(2)").innerText =
            `${current + 1}/${total} test running`;
        }
        target[key] = value;
        return true;
      },
    ...
    const timer = setInterval(() => {
      ...
      if (
        Object.keys(testsDetailsData).length ===
        Object.keys(initialTestInfo).length
      ) {
        clearInterval(timer);
      }
    }, 1000);
